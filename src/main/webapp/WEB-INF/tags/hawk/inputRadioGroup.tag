...
<%@ attribute name="label" required="false" description="Applies a label to the radio group. Uses the provided label as a message code for the i18n message bundle. If the label should not be visible, use <code>labelClass='sr-only'</code> to preserve accessibility." %>
<%@ attribute name="required" required="false" %>
<%@ attribute name="id" required="false" description="optional ID for radio container. path will be used as default. this id will also be used to create IDs for <label> tags" %>
...
<edo:inputBase path="${path}" id="${id}" label="${label}" labelCode="${labelCode}" required="${required}" controlGroupClass="${controlGroupClass}"
