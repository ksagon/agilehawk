...
<spring:eval var="newDateFormat" expression="T(java.text.DateFormat).getDateInstance(T(java.text.DateFormat).SHORT, pageContext.response.locale).toPattern().toLowerCase()"/>
<edo:inputBase path="${path}" id="${id}" label="${label}" labelCode="${labelCode}" required="${required}" controlGroupClass="input-date ${controlGroupClass}"
    ...
    <jsp:attribute name="inputField">
        <form:input path="${path}" id="${id}" class="form-control ${inputClass}" autocomplete="off"
                    data-provide="datepicker" data-date-format="${newDateFormat}" data-date-today-highlight="true" data-date-days-of-week-disabled="${daysOfWeekDisabled}" />
    </jsp:attribute>
</edo:inputBase>
