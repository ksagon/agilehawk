...
<%@ attribute name="label" required="false" description="Applies a label to the (non)input. Attempts to use the provided label as a message code for the i18n message bundle, and falls back to using the label itself. If the label should not be visible, use <code>labelClass='sr-only'</code> to preserve accessibility." %>
...
<edo:formControlGroup id="${controlGroupId}" label="${label}" labelCode="${labelCode}" labelClass="${labelClass}" required="${required}" colspan="${colspan}"
